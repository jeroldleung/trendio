...
def fetch(url, theme, field=None):
    param = {
...
def wordcloud(df, word, count):
    ...
    wordcloud = WordCloud(
        font_path="SimHeiBold.ttf", width=800, height=400, background_color="white"
...
if theme := st.sidebar.text_input("Theme", placeholder="Search something"):
    yrs = pd.DataFrame.from_dict(extract(fetch(FILTER_URL, theme, field="Year")))
    years.line_chart(yrs, x="name", y="count", x_label="", y_label="")
    tps = pd.DataFrame.from_dict(extract(fetch(FILTER_URL, theme, field="Type")))
    types.bar_chart(tps, x="name", y="count", x_label="", y_label="", horizontal=True)
    subjs = pd.DataFrame.from_dict(extract(fetch(FILTER_URL, theme, field="Subject")))
